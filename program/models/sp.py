...
class KeyResultArea(models.Model):
    ...
    output_line_ids = fields.One2many(comodel_name="key.result.area.output.lines",
                                      inverse_name="key_result_area_output_id",
...
class KeyResultAreaOutput(models.Model):
    ...
    description = fields.Text(string="Description", required=True)
    key_result_area_output_id = fields.Many2one(comodel_name="key.result.area", string="KRA ID", readonly=True,
                                                required=False, )
    kra_output_indicator_line_ids = fields.One2many(comodel_name="kra.output.indicators",
                                                    inverse_name="kra_output_indicator_id",
                                                    string="Indicator", required=False, )


class KRAOutputIndicators(models.Model):
    _name = 'kra.output.indicators'
    _rec_name = 'output_indicator'

    output_indicator = fields.Many2one(comodel_name="me.indicators", string="Indicator", required=True, )
    what_to_measure = fields.Char(string="What to measure", required=True)
    baseline_value = fields.Char(string="Baseline Value", required=False)
    kra_output_indicator_id = fields.Many2one(comodel_name="key.result.area.output.lines",
                                              string="Output Indicator",
                                              required=False, readonly=True)
    kra_output_unit_definition_line_ids = fields.One2many(comodel_name="kra.output.unit.definition",
                                                          inverse_name="output_unit_definition_line",
                                                          string="Unit Definition", required=False, )


class KRAOutputUnitDefinition(models.Model):
    _name = 'kra.output.unit.definition'
    _rec_name = 'unit'

    unit = fields.Many2one(comodel_name="unit.configuration", string="Unit/Definition", required=True)
    output_unit_definition_line = fields.Many2one(comodel_name="kra.output.indicators", string="Unit/Definition",
                                                  required=False,
                                                  readonly=True)
    output_actual_period_line_ids = fields.One2many(comodel_name="sp.output.actual.period.lines",
...
class SPOutputActualPeriodLines(models.Model):
    ...
    real_actual_value = fields.Integer(string="Actual value", required=False, default=0, readonly=True)
    target_description = fields.Char(string="Target description", required=False)
    output_unit_line_id = fields.Many2one(comodel_name="kra.output.unit.definition", string="Unit/Definition",
                                          required=False, readonly=True)
    output_actual_period_section_line_ids = fields.One2many(comodel_name="sp.output.actual.period.section.lines",
                                                            inverse_name="output_actual_period_section_line",
                                                            string="Section Period", required=False, )


class SPOutputActualPeriodSectionLines(models.Model):
